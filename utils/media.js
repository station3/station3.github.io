import { css } from 'styled-components'
import { keys } from 'lodash'
import { SIZES } from './config'

// const media = keys(SIZES).reduce((result, label) => {
//   let style = (...args) => css`
//     @media only screen and (max-width: ${SIZES[label]}px) {
//       ${css(...args)}
//     }
//   `

//   if (label === 'desk1600') {
//     style = (...args) => css`
//       @media only screen and (min-width: ${SIZES[label]}px) {
//         ${css(...args)}
//       }
//     `
//   }

//   result[label] = style

//   return result
// }, {})



const media = {
  desk1600: (...args) => css`
    @media only screen and (min-width: ${SIZES.desk1400 + 1}px) {
      ${css(...args)}
    }
  `,

  desk1400: (...args) => css`
    @media only screen 
      and (min-width: ${SIZES.desk1200 + 1}px)
      and (max-width: ${SIZES.desk1400}px) {
      ${css(...args)}
    }
  `,

  desk1200: (...args) => css`
    @media only screen 
      and (min-width: ${SIZES.tabletB + 1}px)
      and (max-width: ${SIZES.desk1200}px) {
      ${css(...args)}
    }
  `,

  tabletB: (...args) => css`
    @media only screen 
      and (min-width: ${SIZES.tablet + 1}px)
      and (max-width: ${SIZES.tabletB}px) {
      ${css(...args)}
    }
  `,
  
  tablet: (...args) => css`
    @media only screen 
      and (min-width: ${SIZES.mobile + 1}px)
      and (max-width: ${SIZES.tablet}px) {
      ${css(...args)}
    }
  `,
  
  mobile: (...args) => css`
    @media only screen 
      and (min-width: ${SIZES.mobileS + 1}px)
      and (max-width: ${SIZES.mobile}px) {
      ${css(...args)}
    }
  `,
  
  mobileS: (...args) => css`
    @media only screen and (max-width: ${SIZES.mobileS}px) {
      ${css(...args)}
    }
  `
}

export default media